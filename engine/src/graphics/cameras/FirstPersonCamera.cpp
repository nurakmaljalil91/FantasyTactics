...
bool FirstPersonCamera::_firstMouse = true;
float FirstPersonCamera::_lastX = 0.0f;
float FirstPersonCamera::_lastY = 0.0f;

FirstPersonCamera::FirstPersonCamera(const glm::vec3 &position, const glm::vec3 &worldUp, float yawDegrees,
                                     float pitchDegrees, float movementSpeed, float mouseSensitivity,
                                     float zoomDegrees) {
    // initialize lastX/lastY to a center on first callback
    // you might want to set these to your window’s width/2 and height/2 before showing cursor
}

void FirstPersonCamera::processInput(GLFWwindow *window, float deltaTime) {
    // WASD movement
...
void FirstPersonCamera::mouseCallback(GLFWwindow *window, const double xPosition, const double yPosition) {
    if (_firstMouse) {
...
void FirstPersonCamera::scrollCallback(GLFWwindow *window, double xOffset, const double yOffset) {
    if (const auto firstPersonCamera = static_cast<FirstPersonCamera*>(glfwGetWindowUserPointer(window))) {
